function [alpha1,beta1,beta_21,gamma1,delta1,Lambda1,Kappa1,lambdaFun,kappaFun] = fit_SEIQRDP(Q,R,D,Npop,E0,I0,time,guess,varargin)
...
if isempty(R)
    ...
    kappaMax = guess(9:11)*1.05; % Constrain the guess if no R available
    kappaMin = guess(9:11)*0.95; % Constrain the guess if no R available
    ...
    kappaMax = guess(9:11)*2.0; % bound the guess around the initial fit
    kappaMin = guess(9:11)/2.0; % bound the guess around the initial fit
    lambdaMax = guess(6:8)*2.0; % bound the guess around the initial fit
    lambdaMin = guess(6:8)/2.0; % bound the guess around the initial fit
...
ub = [1, 5, 2, 10, 10, lambdaMax, kappaMax]; % upper bound of the parameters
lb = [0, 0, 0, 0, 0, lambdaMin, kappaMin]; % lower bound of the parameters
% call Lsqcurvefit
...
gamma1 = abs(Coeff(4));
delta1 = abs(Coeff(5));
Lambda1 = abs(Coeff(6:8));
Kappa1 = abs(Coeff(9:11));
...
%% nested functions
    ...
    function [output] = SEIQRDP_for_fitting(para,t0)
        ...
        gamma = abs(para(4));
        delta = abs(para(5));
        lambda0 = abs(para(6:8));
        kappa0 = abs(para(9:11));
        ...
        for ii=1:N-1
            ...
            ST = Y(1,ii)*Y(2,ii);
            F = zeros(7,1);
            F(1:2,1) = [-beta/Npop;beta/Npop].*SI + [-beta_2/Npop;beta_2/Npop].*ST;
        ...
        if max(D)<10
            ...
            try
                ...
                [coeff1,r1] = lsqcurvefit(@(para,t) myFun1(para,t),...
                    guess(9:11),x(~isnan(rate)),rate(~isnan(rate)),[0 0 0],[1 1 100],opt);
                ...
                guess(9:11) = kappaGuess; % update guess
    ...
    function [guess,lambdaFun] = getLambdaFun(tTarget,Q,R,guess)
        ...
        if max(R)<20
            ...
            try
                ...
                [coeff1,r1] = lsqcurvefit(@(para,t) myFun1(para,t),...
                    guess(6:8),x(~isnan(rate)),rate(~isnan(rate)),[0 0 0],[1 1 100],opt);
                [coeff2,r2] = lsqcurvefit(@(para,t) myFun2(para,t),...
                    guess(6:8),x(~isnan(rate)),rate(~isnan(rate)),[0 0 0],[1 1 100],opt);
                ...
                guess(6:8) = lambdaGuess; % update guess
